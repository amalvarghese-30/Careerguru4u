...
import { Lightbulb, Search, Trash2, Filter, Plus, Edit3, X, Save, Video, FileText, ExternalLink } from "lucide-react";
...
export default function ConceptNotesAdminPage() {
    ...
    const [editing, setEditing] = useState<Partial<ConceptNote>>({});
    ...
    const fetchNotes = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/admin/concept-notes?${params}`);
    ...
    const openNew = () => {
        setEditing({ board: "CBSE", class: 10, subject: "", chapter: "", type: "note", title: "", content: "", videoUrl: "" });
        setIsNew(true);
        setShowEditor(true);
    };

    const openEdit = (n: ConceptNote) => {
        setEditing({ ...n });
        setIsNew(false);
        setShowEditor(true);
    };
    ...
    const handleSave = async () => {
        if (!editing.title || !editing.board || !editing.class || !editing.subject || !editing.type) {
            setMessage("Please fill all required fields");
            return;
        }
        if (editing.type === "video" && !editing.videoUrl) {
            setMessage("Video URL is required for video type");
            return;
        }
        setSaving(true); setMessage("");
        try {
            const res = await fetch("/api/admin/concept-notes", {
                method: isNew ? "POST" : "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(isNew ? editing : { _id: (editing as ConceptNote)._id, ...editing }),
            });
            if (res.ok) {
                setShowEditor(false); setEditing({}); fetchNotes();
                setMessage(isNew ? "Note created" : "Note updated");
            } else {
                const data = await res.json();
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">Concept Notes & Videos</h1>
                    <p className="text-slate-500 text-sm mt-1">Manage concept notes and educational videos for all subjects</p>
                </div>
                <button onClick={openNew} className="flex items-center gap-2 px-5 py-2.5 bg-brand-gradient-static text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-all">
                    <Plus className="h-4 w-4" /> Add Note/Video
                </button>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="flex-1 px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-brand-royal">
                            <option value="">All Types</option>
                            <option value="note">Notes</option>
                            <option value="video">Videos</option>
                        </select>
                        <button onClick={fetchNotes} className="px-4 py-2 bg-slate-100 rounded-xl text-sm font-medium hover:bg-slate-200"><Search className="h-4 w-4" /></button>
                    </div>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-white rounded-2xl border border-slate-200">
                    ...
                    <p className="text-slate-500 font-medium">No concept notes found</p>
                    <p className="text-slate-400 text-sm mt-1">Add notes or videos using the button above</p>
                </div>
            ) : (
                <div className="space-y-2">
                    {notes.map((n) => (
                        <div key={n._id} className="p-4 bg-white rounded-xl border border-slate-200 flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3 min-w-0">
                                <div className={`h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0 ${n.type === "video" ? "bg-purple-100" : "bg-amber-100"}`}>
                                    {n.type === "video" ? <Video className="h-5 w-5 text-purple-600" /> : <FileText className="h-5 w-5 text-amber-600" />}
                                </div>
                                <div className="min-w-0">
                                    <p className="text-sm font-semibold text-slate-800 truncate">{n.title}</p>
                                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                                        <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{n.board}</span>
                                        <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">Class {n.class}</span>
                                        <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{n.subject}</span>
                                        {n.chapter && <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{n.chapter}</span>}
                                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${n.type === "video" ? "bg-purple-100 text-purple-700" : "bg-amber-100 text-amber-700"}`}>{n.type}</span>
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-1 flex-shrink-0">
                                {n.type === "video" && n.videoUrl && (
                                    <a href={n.videoUrl} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg text-slate-400 hover:text-brand-royal hover:bg-brand-royal/10">
                                        <ExternalLink className="h-4 w-4" />
                                    </a>
                                )}
                                <button onClick={() => openEdit(n)} className="p-2 rounded-lg text-slate-400 hover:text-brand-royal hover:bg-brand-royal/10"><Edit3 className="h-4 w-4" /></button>
                                <button onClick={() => setDeleteConfirm(n._id)} className="p-2 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>
                            </div>
                        </div>
                    ))}
                </div>
                    ...
                    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-20 px-4">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white rounded-2xl border border-slate-200 w-full max-w-2xl max-h-[85vh] overflow-y-auto shadow-2xl">
                            <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between rounded-t-2xl">
                                <h3 className="font-semibold text-slate-800 text-lg">{isNew ? "Add" : "Edit"} Concept Note / Video</h3>
                            ...
                            <div className="p-6 space-y-4">
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    <div>
                                        ...
                                        <select value={editing.type || "note"} onChange={(e) => setEditing({ ...editing, type: e.target.value as "note" | "video" })} className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-brand-royal">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    <div>
                                        ...
                                        <input type="text" value={editing.title || ""} onChange={(e) => setEditing({ ...editing, title: e.target.value })} placeholder="e.g. Laws of Motion" className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-brand-royal" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-500 mb-1">Chapter</label>
                                        <input type="text" value={editing.chapter || ""} onChange={(e) => setEditing({ ...editing, chapter: e.target.value })} placeholder="e.g. Chapter 9: Force and Laws of Motion" className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-brand-royal" />
                                    </div>
                                </div>
                                {editing.type === "video" ? (
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-500 mb-1">Video URL *</label>
                                        <input type="url" value={editing.videoUrl || ""} onChange={(e) => setEditing({ ...editing, videoUrl: e.target.value })} placeholder="https://www.youtube.com/embed/..." className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-brand-royal" />
                                    </div>
                                ) : (
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-500 mb-1">Content (HTML/Markdown)</label>
                                        <textarea value={editing.content || ""} onChange={(e) => setEditing({ ...editing, content: e.target.value })} rows={8} placeholder="Write concept notes here..." className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-brand-royal resize-none" />
                                    </div>
                                )}
                            </div>
                ...
                {deleteConfirm && (
                    <div className="fixed inset-0 z-[110] flex items-center justify-center px-4">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white rounded-2xl border border-slate-200 p-6 w-full max-w-sm shadow-2xl text-center">
                            ...
                            <h3 className="font-semibold text-slate-800 mb-2">Delete?</h3>
